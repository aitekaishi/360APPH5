<template>
	<div class="yitixianindexname" style="width:100vw;overflow-x: hidden;height: 100vh;">
		...
		<group class="cardmanageindex cardmanageindexazzz">
		...
		<div v-show="statea==1||statea==3||statea==4||statea==6||statea==7" class="cardbutton">
    		<x-button class="XB default" @click.native="confirm()" type="warn">终止还款计划</x-button>
    	</div>
    	<div style="height: 50px;"></div>
		...
		methods: {
			...
			*/
			...
			confirm(){
				//手动终止通联代还计划接口
				let _this = this
				axios.get(api + '/user/repay/stop/plan?orderNo='+ localStorage.getItem("orderNo") + '&uid=' + localStorage.getItem("uid") + '&token=' + localStorage.getItem("token") + '&valid=0' + globalPM).then(function (res) {
			        console.log(res.data)
			        if (res.data.code === 0) {
			        	_this.errorStatus = true
						_this.errorMsg = res.data.msg
						axios.get(api + '/user/repay/getPlanInfo?orderNo='+ localStorage.getItem("orderNo") + '&uid=' + localStorage.getItem("uid") + '&token=' + localStorage.getItem("token") + '&valid=0' + globalPM).then(function (response) {
