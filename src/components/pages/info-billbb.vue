<template>
	<div class="infobillbb" style="height: 100vh;width: 100vw;">
		...
		<div class="aqirebilla" style="text-align: center;margin-top: 40px;">
    		<img src="../../assets/web/Zsucesss.png" alt="支付成功" style="width: 60px;"/><br /><br />
    		<p>支付成功</p>
		</div>
		...
		created(){
			...
			axios.post(api + '/user/payment/info?id=' + localStorage.getItem('id') + '&uid=' + localStorage.getItem('uid') + '&token=' + localStorage.getItem('token') + '&valid=0' + globalPM).then(function (res) {
				...
				if(res.data.code === 0) {
					...
					_this.valueb = '￥'+(parseFloat(res.data.data.settleAmount)/100).toFixed(2);
					_this.valuec = '￥'+(parseFloat(res.data.data.proFee)/100).toFixed(2);
					...
					_this.valueg = res.data.data.payTime;
