<template>
	<div class="tichenindex" style="width:100vw;height: 100vh;text-align: center;overflow-x: hidden;">
		<loading :show="loading"></loading>
		<toast v-model="errorStatus" type="text">{{errorMsg}}</toast>
		<x-header :left-options="{showBack: true,backText: '',preventGoBack:true}" @on-click-back="back">提成收益</x-header>
		...
		<div style="background: #FFF;height: 55px;border-bottom: 1px solid #F8F8F8;padding: 0 20px;" v-for="item in payment" :key="item.id">
  			<div>
  				<span style="position: relative;float: left;top: 5px;font-size: 14px;">用户{{item.phone}}-开通会员</span><br />
  			...
  			<div style="text-align: right;float: right;position: relative;top: -10px;">
  				<span style="color: #49CB5F;">+&nbsp;{{(item.income)}}</span>
  			</div>
  		</div>
...
<script>
	...
	export default {
		...
		created() {
			//用户提成收益列表
			let _this = this
			axios.get(api + '/user/wallet/getInviteIncomeList?pageNo=1&pageSize=1000000' + '&uid=' + localStorage.getItem("uid") + '&token=' + localStorage.getItem("token") + '&valid=0' + globalPM).then(function (response) {
